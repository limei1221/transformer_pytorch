...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, seq_len, dropout):
        ...
        _2i = torch.arange(0, d_model, step=2, dtype=torch.float32)  # i: dimension
        div_term = torch.exp(_2i * (-math.log(10000.0) / self.d_model))  # (d_model / 2)
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, mask, dropout):
        ...
        if mask is not None:
            # Apply the mask before softmax by setting the attention scores to -1e9
            attention_scores = attention_scores.masked_fill(mask == 0, -1e9)
        attention_scores = torch.softmax(attention_scores,
                                         dim=-1)  # (batch, h, seq_len, seq_len) # Apply softmax on the last dimension
    ...
    def forward(self, q, k, v, mask):
        query = self.w_q(q)  # (batch, seq_len, d_model) x (d_model, d_model) -> (batch, seq_len, d_model)
        key = self.w_k(k)  # (batch, seq_len, d_model) x (d_model, d_model) -> (batch, seq_len, d_model)
        value = self.w_v(v)  # (batch, seq_len, d_model) x (d_model, d_model) -> (batch, seq_len, d_model)

        # (batch, seq_len, d_model) -> (batch, seq_len, h, d_k) -> (batch, h, seq_len, d_k)
        query = query.view(q.size(0), -1, self.h, self.d_k).transpose(1, 2)
        key = key.view(k.size(0), -1, self.h, self.d_k).transpose(1, 2)
        value = value.view(v.size(0), -1, self.h, self.d_k).transpose(1, 2)
...
def build_transformer(src_vocab_size, tgt_vocab_size, src_seq_len, tgt_seq_len, d_model=512, N=6, h=8, dropout=0.1,
                      d_ff=2048):
